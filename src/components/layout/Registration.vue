...
import { required, minLength, maxLength, emailerror, wrongpass } from "@/i18n/translation";
...
const passwordRegField = ref('')
const confirmPasswordRegField = ref('')

const rules = computed(() => ({
  emailField: { required, minLength, maxLength, emailerror },
  passwordRegField: { required, maxLength },
  confirmPasswordRegField: { required, maxLength, wrongpass: wrongpass(passwordRegField.value) },
}))
const v = useVuelidate(rules, {emailField, passwordRegField, confirmPasswordRegField})

const submitFormReg = () => {
  v.value.$touch()
  // if (v.value.$error) return
  // alert('Form registration submitted')
  v.value.$reset();
}
...
const regpassShowHide = () => {
  const regpass = document.querySelector('#regpass');
  const regpassinput = document.querySelector('#regpass input');
  const comfirmpassinput = document.querySelector('#comfirmpass input');
  if (regpassinput.getAttribute('type') === 'password') {
    regpassinput.setAttribute('type', 'text');
    comfirmpassinput.setAttribute('type', 'text');
    regpass.classList.add('active');
  } else {
    regpassinput.setAttribute('type', 'password')
    comfirmpassinput.setAttribute('type', 'password');
...
<template>
  <div :class="['form__registration', { active: openRegistration }]">
    <div class="form__registration_block">
      <div class="form__top">
        <h4 class="form__head">{{ $t("registration.head") }}</h4>
        <p class="form__desc text_five">{{ $t("registration.desc") }}</p>
      </div>
      <form @submit.prevent="submitFormReg">
        <Input
          ...
          name="regemail"
          id="regemail"
          :placeholder="$t('registration.emailplace')"
          v-model:value="v.emailField.$model"
          :error="v.emailField.$errors"/>

        <div class="form__password" id="regpass">
          <Input
            :label="$t('registration.pass')"
            name="regpassword"
            id="regpassword"
            :placeholder="$t('registration.passplace')"
            v-model:value="v.passwordRegField.$model"
            :error="v.passwordRegField.$errors"
            type="password"/>
          <a class="form__password_button" @click="regpassShowHide">
        ...
        <div class="form__password_comfirm" id="comfirmpass">
          <Input
            :label="$t('registration.pass')"
            name="confirmpassword"
            id="confirmpassword"
            :placeholder="$t('registration.passplace')"
            v-model:value="v.confirmPasswordRegField.$model"
            :error="v.confirmPasswordRegField.$errors"
            type="password"/>
        </div>

        <button type="submit" class="btn btn__primary form__button btn__login">{{ $t("registration.button") }}</button>
      </form>
      <div class="form__bottom">
        <p class="form__desc text_five">{{ $t("registration.have") }}</p>
        <a class="form__registration_button" @click="toggleReg">{{ $t("registration.login") }}</a>
      </div>
    </div>
  </div>
</template>
...
.form__registration_block::-webkit-scrollbar {
  display: none;
}

.form__registration_block::-webkit-scrollbar-thumb {
  display: none;
}
